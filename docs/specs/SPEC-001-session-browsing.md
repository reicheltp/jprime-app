...
5. **WHEN** the attendee selects a track filter **THEN** the system **SHALL** show only sessions belonging to that track.
6. **WHEN** the attendee selects a day filter **AND** the conference spans multiple days **THEN** the system **SHALL** show only sessions scheduled on that day.
...
- The session list must be performant for up to 200 sessions (use a virtualised list).
